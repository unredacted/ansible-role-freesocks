...
**Version 2 does one thing: the machine.** It installs Docker and the node container, Caddy and a decoy site on a front node, and tells FCP "here I am". FCP owns everything on the panel (the config profile, the inbounds, the squads, the Hosts), the origin DNS record of a front node, the fronting (Cloudflare, Fastly, L4 edges) and the release to members. The role never talks to the panel: the panel's node secret comes from FCP with the machine configuration. This is the bootstrap contract v2 (FCP `docs/servers.md`, "Node lifecycle").

Version 1 (Outline, the relay-registering role, hostname rotation, panel-to-panel migration, the role writing the panel) lives on the `v1` tag.

## Node purposes

| `node_purpose` | What the machine runs                                        | What FCP does after                                 |
| -------------- | ------------------------------------------------------------ | --------------------------------------------------- |
| `direct`       | REALITY on the public port                                   | creates its address for members, tests, releases    |
| `front`        | WebSocket on loopback behind Caddy (TLS for the origin name) | origin DNS record, an L7 edge (Cloudflare or Fastly) |
| `relay`        | REALITY on the public port, reached through an L4 edge       | an L4 edge in front of it                           |

A node never reaches a member before an operator approves it in Admin -> Servers.

## Requirements

- Debian 12 or 13, Python 3, ports **80 and 443 free** on a front node (Caddy: HTTP-01 on 80, TLS on 443)
...
```sh
# a direct node
ansible-playbook playbook.yml -e target=node1 -e operation_mode=deploy -e node_purpose=direct

# a front node (FCP makes its origin name; or set node_origin_hostname)
ansible-playbook playbook.yml -e target=node2 -e operation_mode=deploy -e node_purpose=front
...
| `remnawave_caddy_email`               |                        | ACME registration (front)                                                 |
